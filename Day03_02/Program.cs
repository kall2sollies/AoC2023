...
public class Program
{
    public static void Main()
    {
        ...
        List<Cell> stars = map.Stars;

        int total = 0;

        foreach (Cell star in stars)
        {
            List<Number> numbers = star.NeighbourNumbers;

            if (numbers.Count == 2)
            {
                Console.WriteLine(string.Join('*', numbers));

                int product = numbers[0].Value * numbers[1].Value;
...
public class Map
{
    ...
    public List<Cell> Stars { get; set; } = new();
...
public class Number
{
    ...
    public string UniqueKey => $"{StartCell.X}.{StartCell.Y}.{Value}";
    ...
    public bool ContainsCell(Cell cell) => NumberCells.Any(nc => nc.X == cell.X && nc.Y == cell.Y);
...
public class Cell
{
    ...
    public List<Number> NeighbourNumbers
    {
        get
        {
            List<Cell> neighbours = Neighbours;
            List<Number> numbers = _map.Numbers;

            Dictionary<string, Number> uniqueNeighbourNumbers = new();

            foreach (Cell cell in neighbours)
            {
                List<Number> neighbourNumbers = numbers.Where(x => x.ContainsCell(cell)).ToList();

                foreach (Number neighbourNumber in neighbourNumbers)
                {
                    uniqueNeighbourNumbers.TryAdd(neighbourNumber.UniqueKey, neighbourNumber);
                }
            }

            return uniqueNeighbourNumbers.Values.ToList();
        }
    }
